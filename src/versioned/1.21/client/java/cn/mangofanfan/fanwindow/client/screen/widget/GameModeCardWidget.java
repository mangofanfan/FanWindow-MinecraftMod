...
public class GameModeCardWidget extends ClickableWidget {
    ...
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        // 更改绘制顺序，先画底色，再画纹理，再画上层
        context.fill(getX() + 1, getY() + 1, getX() + width - 1, getY() + height - 1, bgColor);
        context.drawTexture(modePicture, getX() + 1, getY() + 1,  0, 0, width - 2, height - 2, width - 2, height - 2);
        context.fill(getX() + 1, getY() + 71, getX() + width - 1, getY() + height - 1, strongBgColor);
        context.drawTextWrapped(textRenderer, strongText, getX() + 5, getY() + 76, width - 12, strongColor);
